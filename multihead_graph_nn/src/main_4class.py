...
def main():
    ...
    model = MultiHeadGSGAT(in_dim=len(dataset[0].x[0]), edge_feat_dim=1, hidden_dim=64, num_actions=4).to(device)
    
    checkpoint_path = "MultiHead_GSGAT_4class.pth"
    ...
    torch.save(model.state_dict(), "MultiHead_GSGAT_4class.pth")
    print("Model weights saved.")    
    results_df = pd.DataFrame.from_dict(training_results)
    results_path = f"../docs/multihead_training_results/MultiHeadGSGAT4class_{year}_{month}_{day}"
