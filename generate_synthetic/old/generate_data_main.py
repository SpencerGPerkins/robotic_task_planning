    ...
    for d in range(num_files):
        ...
        for wire in vision_file["wires"]: # Get all wire coords
            if wire["state"] == "on_table":
                table_wirepoints.append(torch.tensor(wire["position"]))
            elif wire["state"] == "held":
                held_wirepoints.append(torch.tensor(wire["position"]))
            elif wire["state"] == "inserted":
                inserted_wirepoints.append(torch.tensor(wire["position"]))
            else:
                raise ValueError("Unkown state found in wires...")
        term_keys = list(vision_file["terminals"].keys())
        for key in term_keys:
            terminalpoints.append(torch.tensor(vision_file["terminals"][key]["position"]))
            
    return table_wirepoints, held_wirepoints, inserted_wirepoints, terminalpoints

def main():
    # Get reference dataset for distributions
    vision_pth = "../dataset/0701_clean/vision/"
    ...
    for key, value in vis["terminals"].items():
        terminal_coords.append(value["position"])   

    # Process reference dataset, generate max-likelihood for distributions
    table_wirepoints, held_wirepoints, inserted_wirepoints, terminal_points = process_file(vision_pth, num_files=276)
    ...
    inserted_mean_vector, inserted_covariance_mat = get_max_likelihood(torch.stack(inserted_wirepoints))
    ...
    trunc_inserted_dist = TruncatedMVN(inserted_mean_vector.cpu().numpy(), inserted_covariance_mat.cpu().numpy(), lb=lb, ub=ub_term)
