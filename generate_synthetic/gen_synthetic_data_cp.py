...
class SyntheticData:
    ...
    def generate_pick_vision(self, num_samples, start_samp_num):
        for n in range(num_samples):
            ...
            for w in range(num_wires):
                ...
                # Save target wire information for label file
                if w == target_wire_idx:
                    label_wire = {
                        "ID": w,
                        "name": f"{color}_wire",
                        "color": color,
            ...
            for t, term_key in enumerate(self.terminals):
                ...
                if term_key == target_terminal:
                    label_terminal = {
                        "name": term_key,
                        "coordinates": self.terminal_coords[t]
                    }
            # Write the label file
             self.save_labels({"target_wire": label_wire, "target_terminal": label_terminal, "correct_action": "pick"}, sample_number=n)
    ...
    def save_labels(self, labels_dict, sample_number):
        with open(f"../dataset/generated_synthetic_dataset/lables/sample_{n}.json", "w") as out_label:
            json.dump(labels_dict, out_label)
        print(f"Sample_{n} saved to ../dataset/generated_synthetic_dataset/labels/sample_{n}.json")    
