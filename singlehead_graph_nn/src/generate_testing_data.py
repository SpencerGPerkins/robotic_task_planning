...
for d in range(200):
    ...
    with open(f"{BASE_DIR}/vision_test/test_sample_{d}.json", "w") as f:
        json.dump(vision_data, f, indent=2)
    with open(f"{BASE_DIR}/llm_test/test_sample_{d}.json", "w") as f:
        json.dump(llm_data, f, indent=2)
    with open(f"{BASE_DIR}/labels_test/test_sample_{d}.json", "w") as f:
        json.dump(y_labels, f, indent=2)

print("50 synthetic testing samples generated and sorted into separate directories.")