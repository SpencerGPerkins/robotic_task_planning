...
def train(model, loader, optimizer,criterion, device):
    ...
    for data in loader:
        for batch in loader:
            print(batch.x.shape)       # Total nodes in batch
            print(batch.batch.shape)   # Should match batch.x.shape[0]
            print(batch.ptr.shape)     # Should be batch_size + 1
            break
        ...
        # )
        action_logits = model(
            data.x.float(), data.edge_index, data.edge_attr, data.batch
        )
...
def main():
    ...
    val_filenames = [data.file_name for data in val_data if hasattr(data, 'file_name')]
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = GraphSAGE_GAT(in_dim=len(dataset[0].x[0]), edge_feat_dim=1,hidden_dim=16, num_actions=5).to(device)
    checkpoint_path = "SingleHead_GSGAT_5class.pth"
    ...
    print(f"Validation graphs: {val_filenames}")    
    # Save the trained model
    torch.save(model.state_dict(), "SingleHead_GSGAT_5class.pth")
    print("Model weights saved.")    
    results_df = pd.DataFrame.from_dict(training_results)
    results_df.to_csv("../docs/training_results/GSGAT_5class_0509.csv")
